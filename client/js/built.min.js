...
var lastUpdate = Date.now(), minInterval = 15, canHandleOrientation;

var cubeEnabled = true, compassEnabled = false;

$(document).ready(function() {
    var HOST = "192.168.0.106", socket = io.connect("http://" + HOST);
    var $cube = $(".cube"), $hostView = $(".host-specific-view"), $controllerView = $(".controller-specific-view"), $instance = $(".instance"), $cubeWrapper = $(".cube-wrapper"), $compassWrapper = $(".compass-wrapper"), $compassPointer = $(".compass-pointer"), $debugAxisX = $('.debug-axis[data-axis="x"]'), $debugAxisY = $('.debug-axis[data-axis="y"]'), $debugAxisZ = $('.debug-axis[data-axis="z"]');
    $(".choice, .menu .item a").on("click", function(e) {
        var choice;
        if (choice = $(this).data("choice")) {
            e.preventDefault();
            if ($(".splash-screen")) $(".splash-screen").remove();
            ...
            resetListeners();
            if ($(".room-input-dialog-wrapper")) $(".room-input-dialog-wrapper").remove();
            if ($(".passphrase-dialog-wrapper")) $(".passphrase-dialog-wrapper").remove();
    ...
    function sendOrientatioonUpdate(data) {
        ...
        if (data.alphaKeyboard) orientationState.alpha += data.alphaKeyboard;
        if (data.betaKeyboard) orientationState.beta += data.betaKeyboard;
        if (data.gammaKeyboard) orientationState.gamma += data.gammaKeyboard;
        if (data.alpha) orientationState.alpha = alpha;
        if (data.beta) orientationState.beta = beta;
        if (data.gamma) orientationState.gamma = gamma;
    ...
    function setupHost() {
        ...
        function startMovingCube() {
            ...
            socket.on("controllerInstruction", function(orientation) {
                ...
                if (compassEnabled) handleCompass(orientation.alpha);
    ...
    function changedEnough(a, b, amount) {
        if (Math.abs(a) > Math.abs(b) + amount || Math.abs(a) < Math.abs(b) - amount) return true;
        return false;
    }
